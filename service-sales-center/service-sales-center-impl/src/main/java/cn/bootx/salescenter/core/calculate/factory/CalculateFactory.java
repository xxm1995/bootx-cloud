package cn.bootx.salescenter.core.calculate.factory;

import cn.bootx.common.util.BigDecimalUtil;
import cn.bootx.salescenter.core.activity.entity.Activity;
import cn.bootx.salescenter.core.calculate.cache.CalculateBufferService;
...
public class CalculateFactory {
     ...
     * 构建订单和缓存
     */
    public OrderAndBuffer buildOrderDto(OrderCheckParam orderParam){
        ...
        // 构建订单缓存
        OrderCache orderDto = this.buildOrderDto(orderParam, calculateCache);
     ...
     * 构建订单dto 带缓存
     */
    public OrderCache buildOrderDto(OrderCheckParam orderParam, CalculateCache calculateCache){
        ...
        List<OrderDetailCache> details = orderParam.getDetails()
                .stream()
                .map(param -> this.buildOrderDetailDto(param,calculateCache))
     ...
     * 构建订单明细dto 带缓存
     */
    private OrderDetailCache buildOrderDetailDto(OrderDetailCheckParam param, CalculateCache calculateCache){
